...
def encode_bytes(entropy: bytes) -> str:
    ...
    checksum = sha256(entropy).digest()[0] >> (8 - num_bits_checksum)
...
def decode_phrase(phrase: str) -> bytes:
    ...
    correct_words = []
    for word in words:
        if word not in INDEX_TO_WORD_TABLE:
    ...
    checksum_for_verification = sha256(data).digest()[0] >> (
        8 - num_bits_checksum
    )
...
def expand_phrase(phrase: str) -> str:
    out = []
...
def short_phrase(phrase: str) -> str:
    out = []
    for word in phrase.split():
        out.append(word[:4])
    return ' '.join(out)
...
def get_entropy_bits(num_words: int = 24) -> int:
    """
